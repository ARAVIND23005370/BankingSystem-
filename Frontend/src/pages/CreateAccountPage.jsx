  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-stone-500 text-sm mt-1">Fill in the details below to open a new bank account</p>
      </div>
      ...
      <Card>
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-stone-400">Assign to User (Email)</label>
            <div className="relative">
              <select
                name="email"
                value={form.email}
                onChange={handleChange}
                className="w-full bg-white/5 border border-white/10 rounded-xl text-stone-200 focus:outline-none focus:border-rose-400/50 focus:ring-1 focus:ring-rose-400/30 transition-all duration-200 py-2.5 pl-4 pr-10 text-sm appearance-none cursor-pointer"
              >
                <option value="" className="bg-stone-900">— No user (unassigned) —</option>
                {(users || []).filter(u => u.role !== 'ADMIN').map((u) => (
                  <option key={u.id} value={u.email} className="bg-stone-900">{u.email}</option>
                ))}
              </select>
              <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-stone-500">▾</div>
            </div>
            <p className="text-xs text-stone-600">Select a registered user to link this account to them</p>
          </div>

          {/* Account Type */}
          <div className="space-y-1.5">
            <label className="block text-sm font-medium text-stone-400">Account Type</label>
                ...
                <button
                  ...
                  className={`py-2.5 px-3 rounded-xl text-sm font-medium border transition-all duration-150 ${
                    form.accountType === type
                      ...
                      : 'bg-white/5 border-white/10 text-stone-500 hover:text-stone-300 hover:border-white/20'
                  }`}
                >
                  {type}
                </button>
          ...
          {(form.holderName || form.accountType || form.balance) && (
            <div className="bg-white/3 border border-white/8 rounded-xl p-4 space-y-2">
              <p className="text-xs font-semibold text-stone-600 uppercase tracking-wider mb-3">Account Preview</p>
              {form.email && (
                <div className="flex justify-between text-sm">
                  <span className="text-stone-500">Assigned To</span>
                  <span className="text-rose-400">{form.email}</span>
                </div>
              )}
              <div className="flex justify-between text-sm">
                <span className="text-stone-500">Holder</span>
                <span className="text-stone-200">{form.holderName || '—'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-stone-500">Account #</span>
                <span className="text-stone-200 font-mono">{form.accountNumber || '—'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-stone-500">Type</span>
                <span className="text-stone-200">{form.accountType || '—'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-stone-500">Opening Balance</span>
                <span className="text-amber-400 font-mono">{form.balance ? `$${parseFloat(form.balance).toFixed(2)}` : '—'}</span>
              </div>
            </div>
          )}
