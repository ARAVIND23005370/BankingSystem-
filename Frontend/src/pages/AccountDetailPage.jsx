...
function DetailRow({ label, value, mono = false }) {
  return (
    <div className="flex items-start justify-between py-3 border-b border-white/5 last:border-0">
      ...
      <span className={`text-stone-200 text-sm font-medium text-right ${mono ? 'font-mono' : ''}`}>{value ?? '—'}</span>
    </div>
  );
}
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <button onClick={() => navigate('/accounts')} className="text-stone-500 hover:text-stone-300 text-sm transition-colors flex items-center gap-1.5">
          ← Back
        </button>
        <div className="flex-1">
          <h2 className="text-xl sm:text-2xl font-bold text-white">{acct.holderName || `Account #${id}`}</h2>
          <p className="text-stone-500 text-sm font-mono mt-0.5">{formatAccountNumber(id)}</p>
        </div>
      ...
      {/* Balance hero */}
      <div className="glass rounded-2xl p-4 sm:p-6 border border-amber-500/10 glow-gold relative overflow-hidden">
        <div className="absolute right-0 top-0 w-64 h-64 bg-amber-500/5 rounded-full blur-3xl pointer-events-none" />
        <p className="text-stone-500 text-sm mb-1">Current Balance</p>
        <p className="text-4xl font-bold font-mono text-amber-400">{formatCurrency(acct.balance)}</p>
        <div className="flex items-center gap-3 mt-4">
          ...
          <span className="text-stone-600 text-xs">Account ID: #{String(id).padStart(4, '0')}</span>
        </div>
      </div>
          ...
          <div className="space-y-2">
            {[
              ...
              >
                ...
                <span className="text-stone-300 text-sm font-medium">{action.label}</span>
                <span className="ml-auto text-stone-600 text-xs">›</span>
              </button>
            ))}
          </div>
        </Card>
