  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="animate-slide-up">
        ...
        <p className="text-stone-500 text-sm mt-1">Here's your banking overview for today</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {loading ? (
          ...
          <>
            <StatCard label="Total Balance" value={formatCurrency(totalBalance)} icon={WalletIcon} color="rose" sub="Across all accounts" delay={0} />
            <StatCard label="Total Accounts" value={list.length} icon={UsersIcon} color="lavender" sub="Active accounts" delay={1} />
                ...
                <thead>
                  <tr className="border-b border-white/5">
                    {['Account Holder', 'Type', 'Balance', 'Created'].map((h) => (
                      <th key={h} className={`text-left px-4 sm:px-6 py-3 text-xs font-semibold text-stone-600 uppercase tracking-wider whitespace-nowrap ${(h === 'Created' || h === 'Type') ? 'hidden sm:table-cell' : ''}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {loading ? (
                    <tr><td colSpan={4} className="px-6 py-8 text-center text-stone-600 text-sm">Loading...</td></tr>
                  ) : recent.length === 0 ? (
                    <tr><td colSpan={4} className="px-6 py-8 text-center text-stone-600 text-sm">No accounts found</td></tr>
                  ) : (
                    recent.map((acct, idx) => (
                      <tr
                        key={acct.id}
                      ...
                      >
                        <td className="px-4 sm:px-6 py-3 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-rose-400/20 to-amber-400/20 border border-white/10 flex items-center justify-center text-xs font-bold text-rose-400">
                              {(acct.holderName || 'U')[0].toUpperCase()}
                            </div>
                            <span className="text-stone-300 text-sm font-medium">{acct.holderName || `Account #${acct.id}`}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 sm:px-6 py-3 font-mono text-sm text-amber-400 whitespace-nowrap">{formatCurrency(acct.balance)}</td>
                        <td className="px-4 sm:px-6 py-3 text-stone-500 text-sm whitespace-nowrap hidden sm:table-cell">{formatDate(acct.createdAt)}</td>
                      </tr>
                    ))
                  )}
                </tbody>
        ...
        <div className="space-y-4 stagger-children">
          <Card glow>
            ...
            <div className="space-y-2">
              {[
                { label: 'Create New Account', to: '/accounts/create', icon: '+', color: 'text-rose-400 bg-rose-400/10' },
                { label: 'Manage Users', to: '/users', icon: '👥', color: 'text-violet-400 bg-violet-400/10' },
                ...
                >
                  ...
                  <span className="text-stone-300 text-sm">{action.label}</span>
                  <span className="ml-auto text-stone-600 group-hover:translate-x-1 transition-transform duration-200">›</span>
                </button>
              ))}
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-stone-500 text-sm">Total Accounts</span>
                <span className="text-white font-mono font-semibold">{list.length}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-stone-500 text-sm">Total Funds</span>
                <span className="text-amber-400 font-mono font-semibold">{formatCurrency(totalBalance)}</span>
              </div>
              <div className="h-px bg-white/5" />
              <div className="flex justify-between items-center">
                <span className="text-stone-500 text-sm">Avg Balance</span>
                <span className="text-rose-400 font-mono font-semibold">{formatCurrency(avgBalance)}</span>
              </div>
            </div>
          </Card>
        </div>
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="animate-slide-up">
        ...
        <p className="text-stone-500 text-sm mt-1">Here's your account overview</p>
      </div>

      {/* Stats */}
      {!loading && list.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 stagger-children">
          <StatCard label="My Accounts" value={list.length} icon={BankIcon} color="rose" sub="Linked to you" delay={0} />
          ...
          <StatCard label="Avg Balance" value={formatCurrency(list.length ? totalBalance / list.length : 0)} icon={TrendIcon} color="lavender" sub="Per account" delay={2} />
        </div>
      )}
      ...
      <Card glow>
        ...
        {loading ? (
          ...
          <div className="text-center py-8">
            <p className="text-stone-500 text-lg mb-2">No accounts linked yet</p>
            <p className="text-stone-600 text-sm">Contact your administrator to create and assign an account to you.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {list.map((acct, idx) => (
              <div
                ...
                style={{ animationDelay: `${idx * 0.05}s` }}
              >
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-rose-400/20 to-amber-400/20 border border-white/10 flex items-center justify-center text-sm font-bold text-rose-400">
                  {(acct.holderName || 'U')[0].toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-stone-200 text-sm font-medium">{acct.holderName || `Account #${acct.id}`}</p>
                  <p className="text-stone-600 text-xs font-mono">#{String(acct.accountNumber || acct.id).padStart(4, '0')}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-amber-400 font-mono text-sm font-semibold">{formatCurrency(acct.balance)}</p>
